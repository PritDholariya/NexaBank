...
We need to spin up the local PostgreSQL database along with Redis and Kafka (for future event-driven components). From the `Backend` directory, run:
...
```
*Wait for this to show "Started DiscoveryServerApplication"*
...
Once all services are up and running, you can interact with the backend in a few ways.
...
2. **Approve Application (Admin Only)**
   ...
   - **Action**: Generates IBAN, Client ID, and raw password.
...
## 💡 Checking Logs
We use SLF4J for logging. When running your Spring Boot applications, watch the console logs heavily. You will see detailed tracking for actions like incoming REST requests via the API Gateway and account approvals in the Account Service.

---

### Running Unit Tests
To verify the internal stability of the applications:
```bash
