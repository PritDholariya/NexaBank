...
export default function LoginPage() {
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (requestError instanceof ApiError) {
        ...
        setError(message || "Login failed. Please retry.");
      } else {
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top_left,#e9f0ff_0%,#f8fbff_42%,#ffffff_100%)] px-4 py-16 sm:px-6">
      <div className="mx-auto max-w-md rounded-3xl border border-[#dce5ff] bg-white p-7 shadow-[0_30px_90px_-50px_rgba(30,68,154,0.42)] sm:p-9">
        <p className="text-xs font-extrabold tracking-[0.2em] text-[#4f64cb] uppercase">NexaBank Portal</p>
        <h1 className="mt-3 text-3xl font-extrabold tracking-tight text-[#172856]">Client login</h1>
        <p className="mt-2 text-sm text-[#55668c]">Use the credentials sent after account approval.</p>

        <form onSubmit={onSubmit} className="mt-7 space-y-4">
          <label className="block">
            <span className="mb-1.5 block text-sm font-semibold text-[#304777]">Client ID</span>
            <input
              value={clientId}
        ...
        <div className="mt-6 flex flex-wrap justify-between gap-2 text-sm">
          <Link href="/open-account" className="font-semibold text-[#3057d6]">Open a new account</Link>
          <Link href="/admin/login" className="font-semibold text-[#3057d6]">Admin login</Link>
